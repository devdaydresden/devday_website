...
class DevDataTests(TestCase):
    ...
    def test_create_objects_failure(self):
        ...
        try:
            ...
            pass
        self.assertTrue(
            "FAILED" in self.stdout.getvalue(),
            "should have FAILED, but got: {}".format(self.stdout.getvalue()),
        )

    def subtest_create_admin_user(self):
        self.devdata.create_admin_user()
        u = User.objects.get(email=settings.ADMINUSER_EMAIL)
        self.assertEqual(u.email, settings.ADMINUSER_EMAIL)
    ...
    def subtest_update_site(self):
        ...
        self.assertEqual(site.domain, "devday.de")
        self.assertEqual(site.name, "Dev Data")

    def get_page(self, title):
        return Page.objects.get(
            title_set__title=title, title_set__published=True, publisher_is_draft=False
        )
    ...
    def check_plugin(self, page, slot, plugin_type):
        ...
        self.assertEqual(
            len(plugins), 1, "{} placeholder has exactly one plugin".format(slot)
        )
        self.assertEqual(
            plugins[0].plugin_type,
            plugin_type,
            "{} placeholder is of type {}".format(slot, plugin_type),
        )

    def subtest_create_pages(self):
        self.devdata.create_objects("pages", Page, 3, self.devdata.create_pages)

        index = self.get_page("Deutsche Homepage")
        self.assertEqual(index.languages, "de", "Homepage is German")
        self.assertEqual(
            index.template, "devday_index.html", "Homepage uses correct template"
        )
        ...
        self.assertEqual(sponsoring.languages, "de", "Sponsoring is German")
        self.assertEqual(
            sponsoring.template,
            "devday_no_cta.html",
        ...
        self.assertEqual(impress.languages, "de", "Impress is German")
        self.assertEqual(
            impress.template, "devday_no_cta.html", "Impress uses correct template"
        )
    ...
    def subtest_update_static_placeholders(self):
        ...
        self.assertEqual(np, 1, "Exactly one static placeholder create")

    def subtest_create_talk_formats(self):
        self.devdata.create_objects(
            "talk formats", TalkFormat, 3, self.devdata.create_talk_formats
        )
        formats = TalkFormat.objects.all().order_by("name", "duration")
        self.assertEqual(len(formats), 4, "There are four TalkFormats")
        self.assertEqual(formats[0].name, "Lightning Talk")
        self.assertEqual(formats[0].duration, 10)
        self.assertEqual(formats[1].name, "Vortrag")
        self.assertEqual(formats[1].duration, 30)
        self.assertEqual(formats[2].name, "Vortrag")
        self.assertEqual(formats[2].duration, 60)
        self.assertEqual(formats[3].name, "Workshop")
        self.assertEqual(formats[3].duration, 180)
    ...
    def subtest_update_events(self):
        ...
        for e in events[:-1]:
            ...
            self.assertEqual(e.submission_open, False, "submission not open")
            tf = e.talkformat.filter(id=stdformat.id)
            self.assertTrue(len(tf) == 1, "standard format assigned")
        e = events[-1]
        self.assertEqual(e.registration_open, True, "registration  open")
        self.assertEqual(e.submission_open, True, "submission open")
        tf = e.talkformat.all()
        self.assertTrue(len(tf) == 4, "all formats assigned")

    def subtest_get_committee_members(self):
        count = (
            len(self.devdata.get_committee_members().strip().split("\n")) - 1
        )  # one extra line
        self.assertEqual(count, 7, "Seven users are committee members")
    ...
    def subtest_get_speakers(self):
        count = (
            len(self.devdata.get_speakers().strip().split("\n")) - 1
        )  # one extra line
        self.assertEqual(count, 10, "At least 10 speakers")
    ...
    def subtest_create_speakers(self):
        ...
        self.assertTrue(
            ...
            "about {:d} speakers: actual {}".format(speakers, number_of_speakers),
        )
        self.subtest_get_speakers()

    def subtest_create_talks(self):
        self.devdata.create_objects("talks", Talk, 1, self.devdata.create_talks)
        speakers = 50
        ...
        talks = speakers * 0.95
        events = Event.objects.annotate(ntalk=Count("talk"))
        for e in events:
            self.assertTrue(
                talks * 0.75 <= e.ntalk <= talks * 1.25,
                "about {:d} talks at event {}: actual {}".format(
                    int(talks), e.title, e.ntalk
                ),
            )

    def subtest_create_votes(self):
        event = Event.objects.current_event()
        self.devdata.create_objects("votes", Vote, 1, self.devdata.vote_for_talk)
        number_of_votes = Vote.objects.exclude(talk__event=event).count()
        self.assertEqual(number_of_votes, 0, "No votes for older events")
        number_of_votes = Vote.objects.count()
        number_of_talks = Talk.objects.filter(event=event).count()
        potential_votes = (
            number_of_talks * User.objects.filter(groups__name=COMMITTEE_GROUP).count()
        )
        self.assertTrue(
    ...
    def subtest_create_tracks(self):
        self.devdata.create_objects("tracks", Track, 1, self.devdata.create_tracks)
        # FIXME implement data checks
        ntracks = Track.objects.filter(event=Event.objects.current_event()).count()
        self.assertEqual(ntracks, 0, "No tracks for current event")
        ntracks = Track.objects.filter(
            event=Event.objects.get(title="devdata.17")
        ).count()
        self.assertEqual(ntracks, 5, "5 tracks for devdata.17")
        ntracks = Track.objects.filter(
            event=Event.objects.get(title="devdata.18")
        ).count()
        self.assertEqual(ntracks, 6, "6 tracks for devdata.18")

    def subtest_create_rooms(self):
        self.devdata.create_objects("rooms", Room, 1, self.devdata.create_rooms)
        nrooms = Room.objects.filter(
            event=Event.objects.get(title="devdata.17")
        ).count()
        self.assertEqual(nrooms, 4, "we have 4 rooms for devdata.17")
        nrooms = Room.objects.filter(
            event=Event.objects.get(title="devdata.18")
        ).count()
        self.assertEqual(nrooms, 4, "we have 4 rooms for devdata.18")

    def subtest_create_time_slots(self):
        self.devdata.create_objects(
            "time slots", TimeSlot, 1, self.devdata.create_time_slots
        )
        events = Event.objects.exclude(id=Event.objects.current_event_id()).annotate(
            ntimeslot=Count("timeslot")
        )
        for e in events:
            self.assertEqual(
                e.ntimeslot, 13, "we have 13 time slots for {}".format(Event)
            )

    def subtest_create_talk_slots(self):
        self.devdata.create_objects(
            "talk slots", TalkSlot, 1, self.devdata.create_talk_slots
        )
        events = Event.objects.exclude(id=Event.objects.current_event_id()).annotate(
    ...
    def test_get_name_from_email(self):
        ...
        self.assertEqual(
            self.devdata.get_name_from_email("first.last@devday.de"), "First Last"
        )
    ...
    def test_create_devdata(self):
        ...
        self.assertTrue("OK" in self.stdout.getvalue(), "At least one OK in output")
        self.assertTrue("FAILED" not in self.stdout.getvalue(), "No FAILED in output")
