...
from django.db.models import Avg, Count, F, Max, Min, Prefetch, Sum
...
class CreateTalkView(TalkSubmissionOpenMixin, SpeakerRequiredMixin, CreateView):
    ...
    def get_form_kwargs(self):
        ...
        kwargs["draft_speaker"] = self.speaker
        return kwargs
...
class TalkDetails(DetailView):
    ...
    def get_queryset(self):
        return (
            ...
            .prefetch_related("media", "published_speakers")
        )
...
class TalkListView(ListView):
    ...
    def get_queryset(self):
        qs = (
            ...
            .select_related(
                "track", "event", "talkslot", "talkslot__time", "talkslot__room"
            )
            .prefetch_related("published_speakers")
        )
    ...
    def get_context_data_for_grid(self, context, **kwargs):
        ...
        talk_slots = list(
            ...
            .select_related("talk", "room", "time", "talk__event")
...
class InfoBeamerXMLView(BaseListView):
    ...
    def get_queryset(self):
        ...
        return (
            ...
            .select_related("track", "talkslot", "talkslot__time", "talkslot__room")
...
class CommitteeSubmitTalkComment(CommitteeRequiredMixin, SingleObjectMixin, FormView):
    ...
    def form_valid(self, form):
        ...
        if self.talk_comment.is_visible:
            recipients = [speaker.user.email for speaker in talk.draft_speakers.all()]
            send_mail(
...
class SpeakerTalkDetails(SpeakerRequiredMixin, UpdateView):
    ...
    def get_queryset(self):
        return (
            ...
            .get_queryset()
            .filter(draft_speakers__user=self.request.user)
        )
    ...
    def render_to_response(self, context):
        ...
        try:
            ...
            writer.writerows(
                [
                    [
                        ", ".join([speaker.name for speaker in t.draft_speakers.all()]),
                        ", ".join(
                            [speaker.organization for speaker in t.draft_speakers.all()]
                        ),
                        t.title,
                        t.abstract,
                        t.remarks,
...
class AttendeeTalkFeedbackSummary(StaffUserMixin, ListView):
    ...
    def get_queryset(self):
        talks = (
            ...
            .filter(event=self.event, track__isnull=False, feedback_count__gte=1)
